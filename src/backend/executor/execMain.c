	...
	foreach(l, epqstate->arowMarks)
	{
		...
		if (erm->markType == ROW_MARK_REFERENCE)
		{
			...
			else
			{
				...
				if (!table_fetch_row_version(erm->relation, (ItemPointer) DatumGetPointer(datum), SnapshotAny, slot,
								   NULL))
					elog(ERROR, "failed to fetch tuple for EvalPlanQual recheck");

#ifdef FIXME
				// don't think this is needed here, shouldn't this be done at
				// access time?
				if (HeapTupleHeaderGetNatts(((HeapTuple)tuple)->t_data) <
					RelationGetDescr(erm->relation)->natts)
				{
					HeapTuple copyTuple = tuple;
