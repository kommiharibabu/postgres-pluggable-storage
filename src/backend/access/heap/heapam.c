	...
	for (;;)
	{
		while (linesleft > 0)
		{
			if (ItemIdIsNormal(lpp))
			{
				 ...
				 */
				valid = HeapTupleSatisfies(tuple, snapshot, scan->rs_scan.rs_cbuf);
					...
					ScanKey key)
{
	...
	for (;;)
	{
		while (linesleft > 0)
		{
			 ...
			 * if current tuple qualifies, return it.
			 */
			if (HeapTupleSatisfies(tuple, scan->rs_scan.rs_snapshot, scan->rs_scan.rs_cbuf))
			{
				/*
				 * if current tuple qualifies, return it.
				 */
...
heap_getnextslot(TableScanDesc sscan, ScanDirection direction, TupleTableSlot *slot)
{
	...
	return ExecStoreTuple(&scan->rs_ctup,
						  slot, scan->rs_scan.rs_cbuf, false);
}
